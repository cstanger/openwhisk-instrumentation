const { owInstrumentation, config } = require('./../index.js');
...
test('test proper wrapper call', async () => {
  expect(owInstrumentation(sum)({ a: 1, b: 2 })).resolves.toBe(3);
});

test('test error if function was not passed', () => {
  expect(() => owInstrumentation()({ a: 1, b: 2 })).toThrow(
...
test('Test logged Metric for INPUT', async () => {
  ...
  await owInstrumentation(sum)({ a: 1, b: 2 });
